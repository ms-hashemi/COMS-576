# This module has some minor modifications compared with the version provided by the professor for drawing methods
import math
...
def draw_cspace(ax, cspace, obstacles, tick_step=[1, 1]):
    ...
    @type obstacles: a list [obs_1, ..., obs_m] of obstacles, where obs_i is a list of coordinates
        on the boundary of the i^{th} obstacle.
    """
    # The following two lines are commented out despite being present in the last assignment as we 
    # do not need to explicitely calculate the obstacle in the configuration space (in fact it is 
    # harder than HW4 since the obstacles are given in the real world space, not the configuration
    # space (they were the same in HW4, and drawing simple obstacle boundaries was easy enough))
